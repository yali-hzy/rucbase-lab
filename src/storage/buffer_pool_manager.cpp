...
bool BufferPoolManager::find_victim_page(frame_id_t* frame_id) {
    ...
    // std::cout<<"find_victim_page"<<std::endl;
    if (free_list_.empty()) {
        // std::cout<<"find_victim_page free list empty"<<std::endl;
        if (!replacer_->victim(frame_id)) {
            // std::cout<<"find_victim_page replacer empty"<<std::endl;
            return false;
        }
    } else {
        // std::cout<<"find_victim_page free list not empty"<<std::endl;
...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    // 5.     返回目标页
    // std::cout<<"fetch_page"<<std::endl;
    std::scoped_lock lock(latch_);
    if (page_table_.find(page_id) != page_table_.end()) {
        frame_id_t frame_id = page_table_[page_id];
        // std::cout<<"fetch_page found in page table "<<frame_id<<std::endl;
    ...
    // std::cout<<"fetch_page found in disk "<<frame_id<<std::endl;
...
Page* BufferPoolManager::new_page(PageId* page_id) {
    ...
    frame_id_t frame_id;
    if (!find_victim_page(&frame_id)) {
        // std::cout<<"new_page 0"<<std::endl;
        return nullptr;
    }
    page_id->page_no = disk_manager_->allocate_page(page_id->fd);
    // std::cout<<"new_page "<<page_id->page_no<<std::endl;
    Page* page = &pages_[frame_id];
    if (page->is_dirty()) {
...
void BufferPoolManager::flush_all_pages(int fd) {
    ...
    for (size_t i = 0; i < pool_size_; i++) {
        Page* page = &pages_[i];
        if (page->id_.fd == fd) {
            disk_manager_->write_page(fd, page->id_.page_no, page->get_data(), PAGE_SIZE);
            page->is_dirty_ = false;
        }
    }
}